...
const api = {
  ...
  post: (ep, body, opts = {}) =>
    apiFetch(ep, { method: "POST", body: JSON.stringify(body), ...opts }),
  put: (ep, body, opts = {}) =>
    apiFetch(ep, { method: "PUT", body: JSON.stringify(body), ...opts }),
  delete: (ep, opts = {}) => apiFetch(ep, { method: "DELETE", ...opts }),
  upload: (ep, formData) =>
    apiFetch(ep, { method: "POST", body: formData, headers: {} }),
};
...
const MOCK = {
  ...
  logMessages: [
    ...
    },
    {
      level: "WARN",
      source: "disk-monitor",
      message: "Disk usage at 71% on /var/log partition",
    },
    {
      level: "ERROR",
      source: "auth-service",
      message: "JWT verification failed — expired token for uid:9182",
    },
    {
      level: "INFO",
      source: "cache-layer",
      message: "Evicted 1,240 stale keys from Redis cluster",
    },
    {
      level: "WARN",
      source: "rate-limiter",
      message: "IP 10.0.0.99 flagged — 512 req/min threshold exceeded",
    },
    {
      level: "DEBUG",
      source: "log-processor",
      message: "Pattern match scan completed in 85ms — 0 new hits",
    },
    {
      level: "ERROR",
      source: "ml-detector",
      message: "Confidence score 0.97 — anomaly confirmed [cluster-3]",
    },
    {
      level: "INFO",
      source: "db-connector",
      message: "Connection pool restored — 10/10 connections active",
    },
    {
      level: "WARN",
      source: "memory-monitor",
      message: "Heap usage at 78% — approaching threshold (85%)",
    },
    {
      level: "ERROR",
      source: "firewall",
      message: "Blocked 4,210 requests from subnet 185.220.0.0/16",
    },
  ],
};
